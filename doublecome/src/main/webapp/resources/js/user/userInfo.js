		...
		for(i = 0; i < result.length; i++){
			...
			$('#bidList').append(
				...
				<a id="aution" href="/doublecome/auction/detailAuction.do?no=${result[i].auctionNo}&userEmail=${result[i].userEmail}">
					...
					<div class="aution_title">
					<div class="card_autionTitle">${result[i].auctionTitle}</div>
					</div>
					<div class="card_sub_now">즉시 구매 가:
					<span class="card_content">${numberWithCommas(result[i].auctionBuyNow)}</span>원
					</div>
					<div class="card_sub_date">남은 시간:
					<span id="remainTime${result[i].auctionCnt}" 
					class="card_content">${format(result[i].auctionLimitDate, "Mdhms")}</span>
					</div>
				</a>				
				</div>`
			);
