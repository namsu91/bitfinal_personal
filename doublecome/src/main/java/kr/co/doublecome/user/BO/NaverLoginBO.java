...
public class NaverLoginBO {
    ...
    private final static String NCLIENT_ID = "O_3IUS2AWHSYnz5QTbOA";       //네이버API Client ID
    private final static String NCLIENT_SECRET = "IXXlbFku7R";                      
    private final static String NREDIRECT_URI = "http://localhost:80/doublecome/user/callback.do";
    ...
    public String getAuthorizationUrl(HttpSession session) {
        ...
        setSession(session,state);     																				/* 생성한 난수 값을 session에 저장 */   
         oauthService = new ServiceBuilder(NCLIENT_ID) 																		/* Scribe에서 제공하는 인증 URL 생성 기능을 이용하여 네아로 인증 URL 생성 */                                                  
                .apiKey(NCLIENT_ID)
    ...
    public OAuth2AccessToken getAccessToken(
        ...
        if(StringUtils.pathEquals(sessionState, state)){
            OAuth20Service oauthService = 
            		new ServiceBuilder(NCLIENT_ID)
                    .apiKey(NCLIENT_ID)
                    .apiSecret(NCLIENT_SECRET)
                    .callback(NREDIRECT_URI)
                    .state(state)
                    .build(NaverLoginApi.instance());
            OAuth2AccessToken accessToken = oauthService.getAccessToken(code); 													/* Scribe에서 제공하는 AccessToken 획득 기능으로 네아로 Access Token을 획득 */
            return accessToken;	
        }
        return null;
    }
    ...
    public String getUserProfile(OAuth2AccessToken oauthToken) throws Exception{
        OAuth20Service oauthService =new ServiceBuilder(NCLIENT_ID)
                .apiKey(NCLIENT_ID)
                .apiSecret(NCLIENT_SECRET)
                .callback(NREDIRECT_URI)
                .build(NaverLoginApi.instance());
            OAuthRequest request = new OAuthRequest(Verb.GET, NPROFILE_API_URL);
